...
void push_front(list1 l, inventory* data)
{
    node1* p = malloc(sizeof(node1));
...
/* void insert(list1 l, node1* pos, inventory* data)
{
    node1* p = malloc(sizeof(node1));
...
/*
// 修改链表中某个节点的数据域
int update(list1 l, char* id, inventory* new_value)
{
    node1* p = l->head->next;
...
/* void traverse(list1 l, int(*visit)(inventory*))
{
    node1* p = l->head->next;
